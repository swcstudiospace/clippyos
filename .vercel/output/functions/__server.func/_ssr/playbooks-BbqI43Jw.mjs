//#region node_modules/.nitro/vite/services/ssr/assets/playbooks-BbqI43Jw.js
var DEFAULT_PLAYBOOK_POLICIES = {
...
var HUMAN_APPROVAL_REQUIRED_FOR = [
	"fee changes",
...
var HERMES_PLAYBOOKS = [
	...
	},
	{
		...
		requiredIntegrations: [],
		risk: "high",
		steps: ["Call get_integration_status", "If AI / Discord / YouTube is down, alert the operator and pause dependent playbooks"],
	...
	},
	{
		...
		summary: "Publish via X, TikTok, and Instagram Graph APIs when connected; Computer Use only for login, CAPTCHA, or ineligible accounts.",
		trigger: "manual",
	...
	},
	{
		...
		summary: "Ingest or search a library asset, caption it, queue a 9:16 render with optional burn-in, then attach to social.create_upload_job via mediaAssetId.",
		trigger: "manual",
	...
	},
	{
		...
		summary: "After social.upload.succeeded, schedule metrics fetch (1h / 24h / 7d). Never invent views. Distill winners to pending KnowledgeProposals — do not auto-merge.",
		trigger: "event",
	...
	},
	{
		...
		summary: "When a social upload fails or needs attention, open a Linear issue in the AI Clipping Dashboard project if Linear is enabled.",
		trigger: "event",
	...
	},
	{
		...
		requiredIntegrations: ["Linear"],
		risk: "low",
		steps: ["Call linear.get_status", "Call linear.create_issue titled [Render] Failed — {preset} with label media"],
		tools: [
			"linear.get_status",
			"linear.create_issue",
			"library.list_renders"
	...
	},
	{
		...
		requiredIntegrations: ["Linear"],
		risk: "low",
		steps: ["Call knowledge.list_proposals", "If Linear autoIssueOnProposal is on, linear.create_issue with label learning"],
...
function parsePlaybookPolicies(raw) {
	...
	return {
		...
		autoAdvanceStageWithoutEvidence: row.autoAdvanceStageWithoutEvidence === true || row.auto_advance_stage_without_evidence === true,
		autoCreateClientFromClosedLead: row.autoCreateClientFromClosedLead === true || row.auto_create_client_from_closed_lead === true,
		analyticsPullConcurrency: row.analyticsPullConcurrency === "medium" || row.analytics_pull_concurrency === "medium" ? "medium" : "low",
		socialAutoStartForUpload: row.socialAutoStartForUpload === true || row.social_auto_start_for_upload === true,
		socialDefaultUploadMode: modeRaw === "publish" ? "publish" : "draft",
		socialMaxAutoRetries: retriesRaw === 2 ? 2 : 1,
		socialMaxBulkJobsPerRun: Number.isFinite(bulkRaw) && bulkRaw >= 1 && bulkRaw <= 20 ? Math.floor(bulkRaw) : 5,
		socialIdleStopMinutes: Number.isFinite(idleRaw) && idleRaw >= 5 && idleRaw <= 240 ? Math.floor(idleRaw) : 20,
		socialRequireLoggedInPlatformsOnly: row.socialRequireLoggedInPlatformsOnly === void 0 && row.social_require_logged_in_platforms_only === void 0 ? true : row.socialRequireLoggedInPlatformsOnly === true || row.social_require_logged_in_platforms_only === true,
		skillsAutoPublishAgent: row.skillsAutoPublishAgent === true || row.skills_auto_publish_agent === true,
		...
		skillsProposeOnAgentSuccess: row.skillsProposeOnAgentSuccess === void 0 && row.skills_propose_on_agent_success === void 0 ? true : row.skillsProposeOnAgentSuccess === true || row.skills_propose_on_agent_success === true
	};
}
function formatPlaybookBrief(policies, enabled) {
...
MCP / API tools
${Array.from(new Set(HERMES_PLAYBOOKS.flatMap((row) => row.tools))).join(", ")}
...
function formatPlaybookPackage(input) {
	...
	const reactors = HERMES_PLAYBOOKS.filter((row) => row.id.startsWith("reactor_")).map((row) => `- ${row.id}: trigger ${row.triggerDetail}`).join("\n");
	const skillLines = (input.skills ?? []).map((row) => `- ${row.slug}@${row.version} — ${row.name}`).join("\n");
	return `${`ClippyOS — Hermes Playbook package
...
//#endregion
export { parsePlaybookPolicies as a, formatPlaybookPackage as i, HERMES_PLAYBOOKS as n, PLAYBOOK_POLICY_LABELS as r, DEFAULT_PLAYBOOK_POLICIES as t };
