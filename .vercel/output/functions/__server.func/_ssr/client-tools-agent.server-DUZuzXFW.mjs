import { G as parseSuggestedIdeas, K as parseSuggestedTitles, s as IDEAS_SYSTEM_PROMPT, y as TITLES_SYSTEM_PROMPT } from "./mappers-Bmic_hyw.mjs";
import { i as routedText } from "./llm-router.server-TNnMY3uU.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/client-tools-agent.server-DUZuzXFW.js
...
async function generateTitleSuggestions(params) {
	...
	if (!params.videos.length) return {
		generatedAt: stamp,
		longFormCount: 0,
		groups: []
	};
	const videoBlock = params.videos.map((video, index) => `${index + 1}. id=${video.videoId} | published=${video.publishedAt ?? "unknown"} | duration=${video.durationSeconds}s | title=${video.title}`).join("\n");
	const operational = [
	...
	const byId = new Map((parsed?.groups ?? []).map((group) => [group.originalVideoId, group.alternatives]));
	...
	return {
		generatedAt: stamp,
		longFormCount: params.videos.length,
		groups
	};
}
async function generateIdeaSuggestions(params) {
	const stamp = (/* @__PURE__ */ new Date()).toISOString();
	const operational = [
		"Return JSON only of the form:",
...
//#endregion
export { generateIdeaSuggestions, generateTitleSuggestions };
