...
//#region node_modules/.nitro/vite/services/ssr/assets/onboarding-CLf2k10Z.js
/** Trusted ClippyOS client agreement — generated server-side, never from user URLs. */
var TITLE = "CLIPPY ADMIN — CLIENT SERVICES AGREEMENT";
var PARAGRAPHS = [
...
function pdfEscape(text) {
	return text.replace(/\\/g, "\\\\").replace(/\(/g, "\\(").replace(/\)/g, "\\)").replace(/[^\x20-\x7E]/g, (char) => {
...
function wrapLine(text, width) {
	...
	for (const word of words) {
		...
		if (word.length > width) for (let i = 0; i < word.length; i += width) {
...
function buildPageContent(lines, pageNumber, pageCount) {
	...
	let first = true;
	for (const line of lines) if (first) {
		commands.push(`(${pdfEscape(line)}) Tj`);
		first = false;
	} else commands.push("0 -14 Td", `(${pdfEscape(line)}) Tj`);
	commands.push("ET");
	commands.push("BT", "/F1 9 Tf", "54 36 Td", `(Page ${pageNumber} of ${pageCount}  ·  ClippyOS  ·  Confidential) Tj`, "ET");
	return commands.join("\n");
}
function buildClientAgreementPdf() {
	const wrapped = [];
...
var CLIENT_AGREEMENT_FILENAME = "Clippy-Admin-Client-Agreement.pdf";
var downloadClientAgreement_createServerFn_handler = createServerRpc({
	id: "3b2ecd259ba82fff78d662e8ed92b4e731d24a7f4b963eef3e6e4896d9f04955",
	name: "downloadClientAgreement",
	filename: "src/lib/server/onboarding.ts"
}, (opts) => downloadClientAgreement.__executeServer(opts));
var downloadClientAgreement = createServerFn({ method: "GET" }).middleware([authMiddleware]).handler(downloadClientAgreement_createServerFn_handler, async () => {
...
//#endregion
export { downloadClientAgreement_createServerFn_handler };
