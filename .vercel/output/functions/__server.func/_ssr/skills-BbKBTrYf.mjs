//#region node_modules/.nitro/vite/services/ssr/assets/skills-BbKBTrYf.js
var SKILLS_QUERY_KEY = ["skills"];
var SKILL_RUNS_QUERY_KEY = ["skill-runs"];
...
function stripSecretsFromSkillText(text) {
	return text.replace(/dtn_[a-zA-Z0-9]+/g, "[redacted]").replace(/xai-[A-Za-z0-9_-]{10,}/g, "[redacted]").replace(/sk-[A-Za-z0-9_-]{10,}/g, "[redacted]").replace(/agk_live_[A-Za-z0-9]+/g, "[redacted]").replace(/mcp_[A-Za-z0-9]+/g, "[redacted]").replace(/Bearer\s+[A-Za-z0-9._-]+/g, "Bearer [redacted]");
}
function skillPackageContainsSecrets(text) {
	return /dtn_[a-zA-Z0-9]{8,}|xai-[A-Za-z0-9_-]{12,}|sk-[A-Za-z0-9_-]{12,}|agk_live_[A-Za-z0-9]+|mcp_[A-Za-z0-9]{12,}|Bearer\s+[A-Za-z0-9._-]{12,}/i.test(text);
}
var AGENT_SKILL_PERMISSIONS = [
	"clients:read",
	"social:read",
	"social:write",
	"analytics:read",
...
function clipAgentSkillPermissions(requested) {
	const allow = new Set(AGENT_SKILL_PERMISSIONS);
...
`,
		scripts: { "health.py": `#!/usr/bin/env python3
...
`,
		scripts: { "caption.py": `#!/usr/bin/env python3
import json, sys
raw = sys.stdin.read().strip() or "{}"
try:
    data = json.loads(raw)
...
`,
		...
		templates: { "caption.txt": "{{clientName}} — {{title}}\n\nLong-form clip. Not a Short.\n" },
		scripts: { "orchestrate.py": `#!/usr/bin/env python3
...
//#endregion
export { parseSkillFrontmatter as a, skillPackageContainsSecrets as c, clipAgentSkillPermissions as i, stripSecretsFromSkillText as l, SKILLS_QUERY_KEY as n, skillListItem as o, SKILL_RUNS_QUERY_KEY as r, skillMcpGet as s, BUILTIN_SKILLS as t };
