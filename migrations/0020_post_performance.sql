-- Post-publish performance, asset rollups, and knowledge proposals from winners.
...
create index if not exists post_performance_platform_idx
  on post_performance (platform, captured_at desc);
...
create index if not exists knowledge_proposals_client_idx
  on knowledge_proposals (client_id, created_at desc);
