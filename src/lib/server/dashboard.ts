/**
 * Server-side dashboard derivations with configurable constants.
 * This module loads configuration from app settings with fallbacks to constants.
 */
...
export function guaranteeStatusForDay(
  dayCount: number,
  guaranteeWindowDays: number,
  guaranteeWarningDay: number,
): GuaranteeStatus {
  if (dayCount >= guaranteeWindowDays) return "past_deadline";
  if (dayCount >= guaranteeWarningDay) return "approaching";
  return "on_track";
}
...
export async function deriveDashboardMetrics(
  ...
  for (const client of active) {
    ...
    if (dayCount >= config.guaranteeWarningDay) clientsAtRisk += 1;
  }
...
export async function deriveGuaranteeItems(
  clients: Client[],
  snapshots: AnalyticsSnapshot[],
  today: string,
): Promise<GuaranteeItem[]> {
  const config = await loadConfig();
  ...
  for (const client of clients) {
    ...
    const status = guaranteeStatusForDay(dayCount, config.guaranteeWindowDays, config.guaranteeWarningDay);
    ...
    let label = `Day ${dayCount}/${config.guaranteeWindowDays} — monitor views for ${client.name}`;
    if (viewsIncreased === true) {
      label = `Day ${dayCount}/${config.guaranteeWindowDays} — views are up for ${client.name}`;
    } else if (status === "past_deadline") {
      label = `Day ${dayCount}/${config.guaranteeWindowDays} — refund window for ${client.name}`;
    }
...
export async function deriveStuckStageCount(
  ...
  for (const client of clients) {
    ...
    if (days != null && days >= config.pipelineStallDays) count += 1;
  }
  return count;
}
...
export function deriveRecentActivity(input: {
  ...
  // Use a reasonable default if config not loaded
  return items.slice(0, 8);
}

export async function deriveDashboardAlerts(input: {
  overdueCount: number;
  overdueTotal: number;
  ...
  if (past.length > 0) {
    ...
    alerts.push({
      ...
      title:
        past.length === 1
          ? `${first.name} is past the ${config.guaranteeWindowDays}-day views window`
          : `${past.length} clients are past the ${config.guaranteeWindowDays}-day views window`,
      href:
        past.length === 1
  ...
  if (approaching.length > 0) {
    alerts.push({
      id: "guarantee-warn",
      severity: "warning",
      title: `${approaching.length} client${approaching.length === 1 ? "" : "s"} approaching day ${config.guaranteeWindowDays}`,
      href: { to: "/clients" },
    });
  }
  ...
  if (input.discordConfigured) {
    ...
    const stale = !Number.isFinite(last) || input.nowMs - last > config.discordAgentStaleMs;
    if (stale) {
      alerts.push({
        id: "discord-stale",
        severity: "warning",
  ...
  if (input.overloaded.length > 0) {
    alerts.push({
      id: "capacity",
      severity: "warning",
      title: `${input.overloaded.length} teammate${input.overloaded.length === 1 ? "" : "s"} assigned to more than ${config.capacityOverloadThreshold} clients`,
      href: { to: "/team" },
    });
  }
