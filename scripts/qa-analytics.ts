...
try {
  ...
  await page.locator("#llm").getByRole("heading", { name: "Grok 4.6" }).waitFor();
  await page.locator("#addons-registry").getByRole("heading", { name: "YouTube Data API" }).waitFor();
  await page.locator("#addons-registry").getByRole("heading", { name: "Higgsfield" }).waitFor();
  const grokBtn = page.getByRole("button", { name: /Connect SuperGrok|Disconnect SuperGrok|Platform key/i });
  console.log({
    grokHeading: await page.locator("#llm").getByRole("heading", { name: "Grok 4.6" }).count(),
    youtubeHeading: await page.locator("#addons-registry").getByRole("heading", { name: "YouTube Data API" }).count(),
