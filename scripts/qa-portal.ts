...
interface PortalNotes {
  ...
  homeRequiresLogin?: boolean;
  anonMoneyIsStaffLogin?: boolean;
  settings?: { url: string; panel: boolean; billingGate: boolean };
  clients?: { url: string; heading: boolean; billingGate: boolean };
  clientPanel?: { heading: boolean; invite: boolean; preview: boolean };
  inviteUrl?: string | null;
  portalHome?: { client: boolean; stage: boolean; noMoney: boolean };
  portalUserMoneyBlocked?: boolean;
}
...
async function shot(p: Page, name: string) {
  try {
    ...
    (notes as Record<string, unknown>)[`shot_${name}`] = String(error).slice(0, 160);
  }
}
