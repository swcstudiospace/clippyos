...
for (const theme of []) {
  const dir = join(videoRoot, `splash-${theme}-v3`);
  ...
  await cap.screenshot({
    path: join(outDir, `splash-${theme}.jpg`),
    type: "jpeg",
    quality: 78,
    fullPage: false,
  });
  ...
  } else console.log("NO VIDEO splash", theme);
}

const remaining: { name: string; path: string; ready: RegExp; theme: "dark" | "light" }[] = [
  { name: "inbox", path: "/inbox", ready: /Inbox|Telegram|WhatsApp/i, theme: "light" },
];

for (const shot of remaining) {
  const dir = join(videoRoot, `${shot.name}-${shot.theme}-v3`);
  mkdirSync(dir, { recursive: true });
  const ctx = await themeContext(browser, storageState, shot.theme, dir);
  const cap = await ctx.newPage();
  ...
  await dismiss(cap);
  await cap.screenshot({
    path: join(outDir, `${shot.name}-${shot.theme}.jpg`),
    type: "jpeg",
    quality: 78,
    fullPage: false,
  });
  ...
  const webm = findWebm(dir);
  if (webm) {
    toGif(webm, join(outDir, `${shot.name}-${shot.theme}.gif`), startSec, 4.2);
    console.log("captured", shot.name, shot.theme);
  } else console.log("NO VIDEO", shot.name, shot.theme);
}
