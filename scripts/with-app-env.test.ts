...
function makeWorkspace(appEnvJson?: string) {
...
test("the wrapped command sees an explicit override, not the file value", async () => {
  const { stdout } = await execFileAsync(
    process.execPath,
    ["--experimental-strip-types", WRAPPER, process.execPath, "-e", PRINT_FLAG],
...
test("the wrapper propagates the command's exit code", async () => {
  await assert.rejects(
    execFileAsync(process.execPath, ["--experimental-strip-types", WRAPPER, process.execPath, "-e", "process.exit(3)"]),
    (err: Error & { code?: number }) => err.code === 3,
  );
});
...
test("a signal-killed command is never reported as success", async () => {
  ...
  await assert.rejects(
    ...
    (err: Error & { code?: number; signal?: string }) =>
