 ...
 */
export function parseAppEnv(text: string): Record<string, string> {
  let parsed: unknown;
  try {
...
export function readAppEnv(root: string): Record<string, string> {
  try {
...
function main(argv: string[]) {
  ...
  if (!command) {
    console.error("usage: node scripts/with-app-env.ts <command> [args…]");
    process.exit(2);
  }
