...
import { appEnvPlugin } from "./app-env-plugin.ts";
import {
 ...
 * with `env` the way `scripts/with-app-env.ts` starts it: `loadEnv`
 * prefix-matches `process.env`, so the value has to be there before the server
 * is created.
 */
async function withDevServer(env: string | undefined, body: (devUrl: string) => Promise<void> | void) {
  ...
  try {
    ...
    try {
      ...
      if (!devUrl) throw new Error("dev server did not resolve a local URL");
...
test("a server without the endpoint probes as null, not as agreement", async () => {
  const notFound = (async () => ({ ok: false, text: async () => "Not Found" })) as unknown as typeof fetch;
  assert.equal(await probeDevAuthEnabled("http://127.0.0.1:8081", notFound), null);
  const html = (async () => ({ ok: true, text: async () => "<!doctype html>" })) as unknown as typeof fetch;
...
test("the CLI reports rather than silently passing when run via a symlink", async () => {
  ...
  const error = await promisify(execFile)(process.execPath, [
    ...
    join(link, "check-auth-invariant.ts"),
    "--dev-url",
    "http://127.0.0.1:1",
  ]).catch((err) => err);
