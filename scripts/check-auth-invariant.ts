 ...
 * `scripts/with-app-env.ts`, so they agree by construction — but a dev server
 ...
 *
 * `scripts/browser-smoke.ts` runs the comparison on every smoke; run it
 ...
 */
import { APP_ENV_ROUTE } from "./app-env-plugin.ts";
import { isMainModule, mergeAppEnv, projectRoot, readAppEnv } from "./with-app-env.ts";

const DEFAULT_DEV_URL = "http://127.0.0.1:8080";

/** The predicate `src/lib/auth/{client,server}.ts` apply to the flag. */
export function authEnabledFromEnvValue(value: string | undefined) {
  return value !== "false";
}

/** Verdict from comparing the dev server's flag against the build's. */
export type AuthInvariantResult = {
  status: "indeterminate" | "ok" | "diverged";
  message: string;
};
/**
 ...
 */
export function compareAuthInvariant({
  devAuthEnabled,
  buildAuthEnabled,
}: {
  devAuthEnabled: boolean | null | undefined;
  buildAuthEnabled: boolean;
}): AuthInvariantResult {
  const label = (value: boolean | null | undefined) => (value ? "on" : "off");
  ...
  return {
    ...
    message:
      ...
      "invoking vite directly skips scripts/with-app-env.ts, so the dev server and " +
 ...
 */
export async function probeDevAuthEnabled(devUrl: string, fetchImpl: typeof fetch = fetch) {
  let env;
  try {
...
export function authInvariantWarnings(result: AuthInvariantResult) {
