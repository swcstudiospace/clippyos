...
try {
  ...
  notes.push({
    ...
    slugs: listed.map((row) => row.slug),
  });
  if (skillsRes.status !== 200) throw new Error("GET /api/v1/skills failed");
  if (!listed.some((row) => row.slug === "daily-ops-brief")) {
  ...
  const addonIds = (addonsJson?.data?.addons ?? []).map((row) => row.id);
  ...
  notes.push({
    ...
    catalog: llmJson?.data?.catalog?.map((row) => row.id),
  });
