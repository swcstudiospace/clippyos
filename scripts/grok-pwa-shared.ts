...
/** Request/env-derived scalar that may legitimately be absent (headers, query strings). */
export type MaybeString = string | null | undefined;

/**
 * Baked share-card identity (`src/lib/og/site.json` plus FS-derived stamps).
 * Every field optional — absent keys fall back to document/host/env defaults.
 */
export type OgSiteIdentity = {
  title?: string;
  description?: string;
  type?: string;
  card?: string;
...
/** Head-injection inputs; every field optional with platform defaults. */
...
function unescapeHtml(value: unknown) {
  return String(value)
    .replaceAll("&lt;", "<")
    .replaceAll("&gt;", ">")
    .replaceAll("&quot;", '"')
...
function placeholderCardColor(site: OgSiteIdentity = {}) {
 ...
 */
export function appNameFromHost(hostHeader: MaybeString) {
...
function isVercelSystemHost(host: string) {
  return (
    host === "vercel.app" ||
...
export function publicAppHost(hostHeader: MaybeString) {
  const host = String(hostHeader ?? "")
    .split(",")[0]
    .trim()
    .split(":")[0]
 ...
 */
export function resolvePublicHost(hostHeader: MaybeString) {
  return (
    publicAppHost(process.env?.VITE_PUBLIC_HOSTNAME) || publicAppHost(hostHeader)
  );
}

export function isInstallQuery(url: MaybeString) {
...
export function isDocumentPath(pathname: MaybeString) {
...
export function acceptsHtml(accept: MaybeString) {
...
export function stripInstallParams(url: MaybeString) {
...
export function renderInstallPageHtml(
  template: MaybeString,
  { host, url }: { host?: MaybeString; url?: MaybeString } = {},
) {
...
export function renderWebManifest(hostHeader: MaybeString) {
...
export function readOgSite(cwd = process.cwd()): OgSiteIdentity {
  try {
    const raw = readFileSync(join(cwd, OG_SITE_REL_PATH), "utf8");
    const parsed: unknown = JSON.parse(raw);
    return parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as OgSiteIdentity)
      : {};
  } catch {
    return {};
  }
...
function detectCustomOgCard(cwd = process.cwd(), site: OgSiteIdentity = {}) {
...
export function titleFromDocument(html: MaybeString) {
...
export function siteHasCustomCard(site: OgSiteIdentity = {}) {
 ...
 */
export function resolveOgCardAsset(site: OgSiteIdentity = {}, cwd = process.cwd()) {
  return ogCardPublicPath(cwd) || (detectCustomOgCard(cwd, site) ? String(site.image ?? "").trim() || "/og.jpg" : "");
}

/** Stamp `card=custom` when public/og.jpg or public/og.png is on disk. */
function applyCustomCardFromFs(site: OgSiteIdentity, cwd: string): OgSiteIdentity {
  const disk = ogCardPublicPath(cwd);
  if (!disk) return site;
...
export function stripShareMetaTags(html: string) {
...
export function injectGrokPwaHead(html: string, ctx: HeadContext = {}) {
...
function findHeadClose(buf: Buffer) {
...
export function createHeadInjector(ctx: HeadContext = {}) {
  ...
  return {
    ...
    push(chunk: Uint8Array | string) {
      ...
      pending = [];
      // findHeadClose just located `</head>` at `at`; the anchored rematch cannot fail.
      const closeLen = joined.toString("latin1", at).match(/^<\/head>/i)?.[0].length ?? 0;
    ...
    /** Whatever is still buffered (no `</head>` seen yet). */
    flush() {
