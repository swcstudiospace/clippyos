 ...
 * `scripts/`), the same split `migration-plan.ts` uses for the two appliers.
 *
...
export function settleWithin(
  ...
  const { promise, resolve } = Promise.withResolvers<"ok" | "timeout" | "failed">();
 ...
 * happen.
 * @param steps Sign-out steps.
 * @returns Resolves after the session ended, token cleared, and redirect fired.
 */
 ...
 * @param steps Pre-sign-in steps.
 * @returns Resolves once any prior session teardown settled.
 */
